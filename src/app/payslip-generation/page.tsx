...
const SlipGeneration = () => {
  ...
  const fetchPayrollData = async (employeeId: string) => {
    try {
      const response = await fetch(`http://localhost:3000/api/payroll?employeeId=${employeeId}`);
      const data = await response.json();
      setPayrollData(data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4">
      <div className="container mx-auto">
        <h1 className="text-2xl font-bold mb-4">Generate Payroll Slip</h1>
        ...
        {payrollData && (
          <div className="border border-gray-700 p-4 rounded-lg">
            <h2 className="text-xl font-bold mb-2">Payroll Slip</h2>
